...
def tfd(n_trials):
    ...
    state = DD()
    with open('tfd_powerup_no_mom_temp.yaml') as ymtmp:
    ...
    learning_rate_scalers = numpy.random.uniform(low=0.01, high=1.1, size=30)
    l2_pen = numpy.logspace(numpy.log10(1e-6), numpy.log10(3*1e-3), 100)
    max_col_norms = [1.8365, 1.9365, 2.1365, 2.2365, 2.3486, 2.4365]

    ind = 0
    TABLE_NAME = "powerup_tfd_1layer_finer_large_no_mom"
